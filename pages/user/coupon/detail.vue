		...
		<view class="con-box" :style="[{'padding-top':customBarH + 'px'}]">
			<view class="ticket-con" :style="{backgroundImage:`url(${bgImg})`}">
				<view class="ticket-con-top">
					<view class="ticket-left" :class="ticketInfo.type==2?'FF9828':''">{{ticketInfo.type == '1'? ('￥' + ticketInfo.money) : ticketInfo.money}}</view>
					<view class="ticket-right" >
						<view class="ticket-top">
							<view class="ticket">{{ticketInfo.type=='1'?'满减券':'折扣券'}}</view>
							<view class="moneyuse">{{ticketInfo.moneyuse}}</view>
						</view>
						<view class="ticket-bottom">有效时间：{{ticketInfo.time}}</view>
					</view>
				</view>
				<view class="ticket-con-bottom">
					...
					<view class="status" :class="ticketInfo.status=='used'?'used':''">{{ticketInfo.status=='unused'?'未使用':'已使用'}}</view>
				</view>
			</view>
			<view class="rank">
				...
				<view class="conttext">
					...
					<view>·该提货码一经兑换，概不退换，最终解释权归大会所有</view>
